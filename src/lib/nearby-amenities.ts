...
export type NearbyPlace = { name: string; distance: number; rating?: string };
...
const PARKS_SEARCH_RADIUS_KM = 5;
const NURSERIES_SEARCH_RADIUS_KM = 5;

// PostgREST caps each response at its configured max-rows (typically 1000)
// regardless of .limit(), so page through with .range() to cover dense urban
// bounding boxes (central London has 1500+ NHS locations in a 5km box).
async function fetchRowsInBoundingBox<T extends { latitude: number | null; longitude: number | null }>(
  table: string,
  columns: string,
  latitude: number,
  longitude: number,
  ...
  for (let page = 0; page < MAX_PAGES; page++) {
    ...
    if (error) {
      console.error(`Failed to fetch rows from ${table}:`, error);
      break;
    }
...
type NhsLocationsResult = {
  hospitals: NearbyPlace[];
  gpSurgeries: NearbyPlace[];
  dentists: NearbyPlace[];
};

function emptyNhsLocations(): NhsLocationsResult {
  return { hospitals: [], gpSurgeries: [], dentists: [] };
}

/** CQC-sourced hospitals/GPs/dentists from Supabase, queried by a lat/lon bounding box then haversine-sorted. */
export async function fetchNhsLocations(
  latitude: number,
  longitude: number,
  radiusKm: number = NHS_SEARCH_RADIUS_KM
): Promise<NhsLocationsResult> {
  try {
    const rows = await fetchRowsInBoundingBox<{
      name: string | null;
      category: string;
      latitude: number | null;
      longitude: number | null;
      overall_rating: string | null;
    }>("nhs_locations", "name, category, latitude, longitude, overall_rating", latitude, longitude, radiusKm);

    const buckets: NhsLocationsResult = emptyNhsLocations();
    for (const row of rows) {
      if (row.latitude == null || row.longitude == null || !row.name) continue;
      const place: NearbyPlace = {
        name: row.name,
        distance: calculateDistanceKm(latitude, longitude, row.latitude, row.longitude),
        rating: row.overall_rating ?? undefined,
      };
      if (row.category === "hospital") buckets.hospitals.push(place);
      else if (row.category === "gp_surgery") buckets.gpSurgeries.push(place);
      else if (row.category === "dentist") buckets.dentists.push(place);
    }

    return {
      hospitals: takeNearestPlaces(buckets.hospitals, 3),
      gpSurgeries: takeNearestPlaces(buckets.gpSurgeries, 3),
      dentists: takeNearestPlaces(buckets.dentists, 3),
    };
  } catch (error) {
    console.error("Error fetching NHS locations from Supabase:", error);
    return emptyNhsLocations();
  }
}

/** OS Open Greenspace-sourced parks from Supabase, queried by a lat/lon bounding box then haversine-sorted. */
export async function fetchOfficialParks(
  latitude: number,
  longitude: number,
  radiusKm: number = PARKS_SEARCH_RADIUS_KM
): Promise<NearbyPlace[]> {
  try {
    const rows = await fetchRowsInBoundingBox<{
      name: string | null;
      latitude: number | null;
      longitude: number | null;
    }>("parks", "name, latitude, longitude", latitude, longitude, radiusKm);

    const places = rows
      .filter((row) => row.latitude != null && row.longitude != null && row.name)
      .map((row) => ({
        name: row.name as string,
        distance: calculateDistanceKm(latitude, longitude, row.latitude as number, row.longitude as number),
      }));

    return takeNearestPlaces(places, 3);
  } catch (error) {
    console.error("Error fetching parks from Supabase:", error);
    return [];
  }
}

/** Ofsted-registered nurseries/preschools from Supabase, queried by a lat/lon bounding box then haversine-sorted. */
export async function fetchNurseries(
  latitude: number,
  longitude: number,
  radiusKm: number = NURSERIES_SEARCH_RADIUS_KM
): Promise<NearbyPlace[]> {
  try {
    const rows = await fetchRowsInBoundingBox<{
      name: string | null;
      ...
      longitude: number | null;
      overall_rating: string | null;
    }>("nurseries", "name, latitude, longitude, overall_rating", latitude, longitude, radiusKm);
    ...
    console.error("Error fetching nurseries from Supabase:", error);
    return [];
  }
}
...
export async function getAllNearbyPlaces(
  ...
  const nhsPlacesPromise = fetchNhsLocations(latitude, longitude);
  const parksPromise = fetchOfficialParks(latitude, longitude);
  const nurseriesPromise = fetchNurseries(latitude, longitude);

  try {
    const [elements, nhsPlaces, officialParks, nurseries] = await Promise.all([
      fetchOverpassAmenities(latitude, longitude),
      nhsPlacesPromise,
      parksPromise,
      nurseriesPromise,
    ]);
    ...
    return {
      ...
      parks: takeNearestPlaces(mergePlacesByName(buckets.park, officialParks), 3),
      supermarkets: takeNearestPlaces(buckets.supermarket, 3),
      placesOfWorship,
      gyms: takeNearestPlaces(buckets.gym, 3),
      hospitals: takeNearestPlaces(mergePlacesByName(buckets.hospital, nhsPlaces.hospitals), 3),
      gpSurgeries: nhsPlaces.gpSurgeries,
      dentists: nhsPlaces.dentists,
      nurseries,
    };
  } catch (error) {
    console.error("Error fetching nearby places from Overpass:", error);
    const nhsPlaces = await nhsPlacesPromise.catch(() => ({
      hospitals: [] as NearbyPlace[],
      gpSurgeries: [] as NearbyPlace[],
      dentists: [] as NearbyPlace[],
    }));
    const officialParks = await parksPromise.catch(() => [] as NearbyPlace[]);
    const nurseries = await nurseriesPromise.catch(() => [] as NearbyPlace[]);
    return {
      ...apifyPlaces,
      parks: officialParks,
      hospitals: nhsPlaces.hospitals,
      gpSurgeries: nhsPlaces.gpSurgeries,
      dentists: nhsPlaces.dentists,
      nurseries,
    };
...
export function mergeNearbyPlaces(
  ...
  return {
    ...
    gpSurgeries: takeNearestPlaces(mergePlacesByName(base.gpSurgeries, enrichment.gpSurgeries), 3),
    dentists: takeNearestPlaces(mergePlacesByName(base.dentists, enrichment.dentists), 3),
    nurseries: takeNearestPlaces(mergePlacesByName(base.nurseries, enrichment.nurseries), 3),
  };
}
...
export function nearbyPlacesToAmenityList(places: NearbyPlacesResult): Array<{
  ...
  const amenities: Array<{ category: string; name: string; distance: number; rating?: string }> = [];
  ...
  for (const [key, category] of mapping) {
    for (const place of places[key]) {
      amenities.push({ category, name: place.name, distance: place.distance, rating: place.rating });
    }
  }
